...
`),L.log(`Sending notification '${P.method}'.`,x)}else Mr("send-notification",P)}function Dn(P,x,j){if(!(b===xe.Off||!L))if(O===un.Text){let z;(b===xe.Verbose||b===xe.Compact)&&(P.error&&P.error.data?z=`Error data: ${gt(P.error.data)}
...
`),L.log(`Received request '${P.method} - (${P.id})'.`,x)}else Mr("receive-request",P)}function In(P){if(!(b===xe.Off||!L||P.method===lm.type.method))if(O===un.Text){let x;(b===xe.Verbose||b===xe.Compact)&&(P.params?x=`Params: ${gt(P.params)}
...
inside top level rule: ->`+t.name+"<-";return r}};En.defaultGrammarValidatorErrorProvider={buildDuplicateFoundError:function(t,e){function r(l){return l instanceof D_.Terminal?l.terminalType.name:l instanceof D_.NonTerminal?l.nonTerminalName:""}var n=t.name,i=(0,I_.default)(e),o=i.idx,a=(0,xx.getProductionDslName)(i),s=r(i),u=o>0,c="->".concat(a).concat(u?o:"","<- ").concat(s?"with argument: ->".concat(s,"<-"):"",`
...
For Further details.`,n},buildEmptyRepetitionError:function(t){var e=(0,xx.getProductionDslName)(t.repetition);t.repetition.idx!==0&&(e+=t.repetition.idx);var r="The repetition <".concat(e,"> within Rule <").concat(t.topLevelRule.name,`> can never consume any tokens.
