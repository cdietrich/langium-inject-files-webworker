import { jq as Q, cv as M, co as Z, t$ as D, u0 as ee, u1 as te, u2 as ie, u3 as se, nG as j, nH as ne, nI as ae, B as u, ts as oe, mS as re, ic as le, aL as ce, tY as $, _ as y, a as d, bC as G, oc as H, u4 as pe, sp as he, u5 as O, u6 as de, u7 as ue, ct as B, iv as ge, hd as fe, iA as Se, N as z, od as me, aV as _e, bA as ve, aU as be, g as xe, f as we, nR as ye, cy as Ie, e as Le, h_ as P, ap as Ce, nN as J, rU as Pe, w as _, bE as Te, u8 as ke, u9 as We, ff as Fe, dl as Ee, dE as Ne } from "./index-6dabdcd0.js";
import { l as Ue } from "./languageService-e48cc0e5.js";
class Re {
  constructor(t) {
    this.isBogous = !1, this.isTrivial = !1, this.usesClipboardVariable = !1, this.usesSelectionVariable = !1, this.codeSnippet = t;
    const e = new M().parse(t, !1), i = /* @__PURE__ */ new Map();
    let n = 0;
...
const q = "_snippet.markAsUsed";
ce.registerCommand(q, (p, ...t) => {
  const e = p.get($), [i] = t;
  i instanceof X && e.updateUsageTimestamp(i);
});
class v {
  constructor(t, e) {
    this.snippet = t, this.label = { label: t.prefix, description: t.name }, this.detail = u(
      ...
      t.source
    ), this.insertText = t.codeSnippet, this.extensionId = t.extensionId, this.range = e, this.sortText = `${t.snippetSource === 3 ? "z" : "a"}-${t.prefix}`, this.kind = 27, this.insertTextRules = 4, this.command = { id: q, title: "", arguments: [t] };
  }
  resolve() {
    return this.documentation = new ge().appendCodeblock("", M.asInsertText(this.snippet.codeSnippet)), this;
  }
...
let k = class {
  ...
  async provideCompletionItems(t, e, i) {
        ...
        t:
          for (let S = Math.max(f, h - o.prefixLow.length); S < l.length; S++) {
            if (!O(l, S, h, o.prefixLow, 0, o.prefixLow.length))
              continue t;
            const U = o.prefixLow.length - (h - S), K = ue(l, o.prefixLow, h, h + U, h - S), I = e.with(void 0, S + 1);
            if (g && e.equals(I))
              continue e;
            let R = K === 0 ? e.column + U : e.column;
            if (h < l.length) {
            ...
            const A = B.fromPositions(I, { lineNumber: e.lineNumber, column: R }), Y = A.setEndPosition(e.lineNumber, e.column);
    ...
    if (!E && (/\s/.test(l[e.column - 2]) || !l))
      for (const m of r) {
        const f = B.fromPositions(e), S = l.indexOf(m.prefixLow, h) === h ? f.setEndPosition(e.lineNumber, e.column + m.prefixLow.length) : f;
...
let x = class W {
  ...
  updateIgnored(t, e) {
    let i = !1;
    this._ignored.has(t) && !e ? (this._ignored.delete(t), i = !0) : !this._ignored.has(t) && e && (this._ignored.add(t), i = !0), i && this._storageService.store(W._key, JSON.stringify(Array.from(this._ignored)), 0, 0);
  }
};
...
let V = class {
  constructor(t, e, i, n, a, s, r, l, g, c, h) {
    ...
    }))), ke(new k(this._languageService, this, h)), this._enablement = c.createInstance(x), this._usageTimestamps = c.createInstance(w);
  }
  dispose() {
  ...
  async getSnippets(t, e) {
    ...
    if (t) {
      ...
      for (const a of this._files.values())
        n.push(a.load().then((s) => We(i, i.length, s.data)).catch((s) => this._logService.error(s, a.location.toString())));
